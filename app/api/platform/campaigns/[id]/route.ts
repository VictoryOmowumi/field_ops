...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: org }, { data: assignments }, { data: visits }, { data: profiles }, { data: outlets }] = await Promise.all([
    ...
    supabase.from("visits").select("id, created_at, outcome, sync_status, outlet_id, agent_id").eq("campaign_id", id).order("created_at", { ascending: false }),
  ...
  const synced = visitsList.filter((v) => v.sync_status === "synced").length;
  const sync = visitsList.length > 0 ? `${((synced / visitsList.length) * 100).toFixed(1)}%` : "-";
  const conversions = visitsList.filter((v) => v.outcome === "converted").length;
